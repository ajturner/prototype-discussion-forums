...
const EntityCard = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.cardSelect = index.createEvent(this, "cardSelect", 7);
    this.handleClick = () => {
  ...
  render() {
    ...
    if (this.compact) {
      return (index.h("button", { class: `card card--compact ${this.selected ? 'card--selected' : ''}`, onClick: this.handleClick, "aria-label": commonName }, index.h("span", { class: "card__photo-mini" }, photoUrl && !this.imgError ? (index.h("img", { src: photoUrl, alt: commonName, onError: this.handleImgError })) : (index.h("span", { class: "placeholder" }, placeholder))), index.h("span", { class: "card__name-mini" }, commonName)));
    }
    return (index.h("div", { class: `card ${this.selected ? 'card--selected' : ''}`, onClick: this.handleClick, role: "button", tabIndex: 0, onKeyDown: e => (e.key === 'Enter' || e.key === ' ') && this.handleClick(), "aria-label": `View details for ${commonName}` }, index.h("div", { class: "card__photo" }, photoUrl && !this.imgError ? (index.h("img", { src: photoUrl, alt: commonName, onError: this.handleImgError })) : (index.h("span", { class: "placeholder" }, placeholder))), index.h("div", { class: "card__body" }, index.h("div", { class: "card__names" }, index.h("span", { class: "card__common" }, commonName), index.h("span", { class: "card__scientific" }, scientificName)), status && index.h("div", { class: "card__status" }, status), iNatUrl && (index.h("a", { class: "card__link", href: iNatUrl, target: "_blank", rel: "noopener noreferrer", onClick: e => e.stopPropagation() }, "View on iNaturalist \u2192")))));
  }
};
EntityCard.style = entityCardCss;

exports.entity_card = EntityCard;

//# sourceMappingURL=entity-card.cjs.entry.js.map